...
function send(userID, userResp) {
	com.sendTextMessage(userID, 'Write 1 sentence action plan on how you will work on the quality you have selected.');
	return null;
}
...
function analyze(userID, userResp, nextSeqs) {
	let 
		...
		error = null;
	if (db.updateCurrentEntry(userID, 'Plan', userResp)['err']) {
